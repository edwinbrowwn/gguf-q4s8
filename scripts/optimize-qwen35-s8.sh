#!/usr/bin/env bash
# Build and compare mixed Q4_0/Q8_0 candidates, then select a Pareto-best recipe.
set -Eeuo pipefail
...
if [[ "$SKIP_BUILD" -eq 0 ]]; then
    for stage in "${STAGE_LIST[@]}"; do
        out="$OUT_ROOT/$stage"
        args=(--repo "$REPO" --build-dir "$BUILD_DIR" --python "$PYTHON"
              --input "$INPUT" --bf16 "$BF16" --out-dir "$out"
