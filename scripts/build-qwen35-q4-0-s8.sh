#!/usr/bin/env bash
# Build a Qwen3.6-35B-A3B mixed Q4_0/Q8_0 recipe from either an existing
# stock Q4_0 tensor map or a fresh BF16 Q4_0 dry-run. The fixed stage promotes
# base Q4_0 tensors selected by the Q4_K_M dry-run. The native stage additionally
...
Options:
  ...
  --build-dir PATH             llama.cpp build directory (default: REPO/build)
...
[[ -d "$REPO" ]] || die "llama.cpp source tree does not exist: $REPO"
...
RANKER="$TOOL_ROOT/scripts/rank-q4-0-q8-promotions.py"
if [[ "$STAGE" == auto ]]; then
